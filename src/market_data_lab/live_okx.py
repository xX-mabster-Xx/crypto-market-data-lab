"""Record public OKX perpetual market data without account credentials.
...
from nautilus_trader.adapters.okx import OKXVipLevel
...
STOP_TIMER = "okx-live-recorder-stop"
PUBLIC_OPEN_INTEREST_PATH = "/api/v5/public/open-interest"


class OKXRecorderConfig(DataActorConfig):
    def __init__(
        self,
...
class OKXRecorder(DataActor):
    """Subscribe, persist with Nautilus's writer, and measure stream quality."""

    def __init__(self, config: OKXRecorderConfig) -> None:
    ...
    def on_start(self) -> None:
        ...
        # In NautilusTrader 2.0.0rc3 a writer created inside a Python actor is
        # not attached to the live node's Rust bus. Explicit callbacks guarantee
        # that persisted records and quality metrics observe the same messages.
    ...
    def on_time_event(self, event: TimeEvent) -> None:
        if event.name == STOP_TIMER:
            self.shutdown_system(f"OKX recorder completed {self._duration_seconds:g}s run")
    ...
    def on_book_deltas(self, deltas: OrderBookDeltas) -> None:
        ...
        snapshot = any(delta.flags & snapshot_flag for delta in deltas.deltas)
        sequence_metric = self._book_sequences.setdefault(
            str(deltas.instrument_id),
            BookSequenceMetric(),
        )
        # OKX provides seqId/prevSeqId rather than Bybit-style consecutive
        # update IDs. The adapter checks pairwise continuity and resnapshots on
        # failure; here we retain cross-sequence regressions as an audit signal.
        sequence_metric.add(
            update_id=None,
    ...
    def _finalize(self) -> None:
        ...
        atomic_json(
            self._stats_path,
            {
                ...
                "streams": {
                    data_type: {
                        instrument: metric.to_dict() for instrument, metric in metrics.items()
                    }
                    for data_type, metrics in self._metrics.items()
                },
                "book_sequences": {
                    instrument: metric.to_dict()
                    for instrument, metric in self._book_sequences.items()
                },
                "books": books,
...
def _public_json_sync(url: str, proxy_url: str | None, timeout_seconds: float) -> Any:
    if proxy_url and not proxy_url.startswith(("http://", "https://")):
        raise ValueError(
            "REST open-interest polling supports direct, HTTP, or HTTPS proxy URLs; "
            "disable it when using a SOCKS proxy",
        )
    handlers: list[Any] = []
    if proxy_url:
...
def parse_open_interest_response(
    ...
    if not isinstance(payload, dict) or str(payload.get("code")) != "0":
        raise ValueError(f"OKX open-interest error response: {payload!r}")
    rows = payload.get("data")
    if not isinstance(rows, list) or not rows:
        raise ValueError(f"Empty open-interest response for {expected_instrument_id}")
    row = next(
        (item for item in rows if item.get("instId") == expected_instrument_id),
        None,
    )
    if row is None:
        raise ValueError(f"Instrument {expected_instrument_id} missing from open-interest response")
    return {
        "instrument_id": row.get("instId"),
        "instrument_type": row.get("instType"),
        "open_interest_contracts": row.get("oi"),
        "open_interest_currency": row.get("oiCcy"),
        "open_interest_usd": row.get("oiUsd"),
...
async def poll_public_rest(
    ...
    fetch_json: Callable[[str, str | None, float], Any] = _public_json_sync,
) -> dict[str, Any]:
    """Poll public OI and estimate the local clock offset from OKX system time."""
    ...
    try:
        while not stop_event.is_set():
            ...
            if clock_output is not None:
                ...
                try:
                    server_time_ms = int(await client.get_server_time())
            ...
            if oi_output is not None:
                for symbol in symbols:
                    ...
                    url = f"https://www.okx.com{PUBLIC_OPEN_INTEREST_PATH}?{query}"
                    request = read_local_clocks()
                    local_clock.add(request)
                    try:
                        payload = await asyncio.to_thread(fetch_json, url, proxy_url, 10.0)
                        receive = read_local_clocks()
                        local_clock.add(receive)
                        record = parse_open_interest_response(payload, symbol)
                        record.update(
                            {
    ...
    return {
        "open_interest": {
            ...
            "path": str(open_interest_path.resolve()),
        },
        "clock_sync": {
            "enabled": not disable_clock_sync,
            "endpoint": "GET /api/v5/public/time",
            "method": (
                "exchange time minus local monotonic-RTT midpoint; "
...
def _parse_symbols(value: str) -> list[str]:
    ...
    valid = all(
        len(symbol.split("-")) == 3
        and symbol.endswith("-SWAP")
        and symbol.split("-")[1] == "USDT"
        for symbol in symbols
    )
    if not symbols or not valid:
        raise argparse.ArgumentTypeError(
            "symbols must be comma-separated OKX USDT swaps, e.g. BTC-USDT-SWAP",
        )
    return symbols
...
async def collect(args: argparse.Namespace) -> dict[str, Any]:
    ...
    instrument_ids = [f"{symbol}.OKX" for symbol in args.symbols]

    node = (
        LiveNode.builder(
            f"OKX-RECORDER-{run_id}",
        ...
        .add_data_client(
            ...
            OKXDataClientConfig(
                ...
                instrument_families=[symbol.removesuffix("-SWAP") for symbol in args.symbols],
                environment=OKXEnvironment.LIVE,
                proxy_url=args.proxy_url,
                vip_level=OKXVipLevel.VIP0,
            ),
        )
        .build()
    )
    node.add_actor_from_config(
        ImportableActorConfig(
            actor_path="market_data_lab.live_okx:OKXRecorder",
            config_path="market_data_lab.live_okx:OKXRecorderConfig",
            config={
                "actor_id": "OKX-RECORDER-001",
    ...
    manifest = {
        ...
        "book_channel": "books (public standard; VIP0)",
        "timestamp_semantics": {
            ...
            "order_book_ts_event": (
                "OKX order-book message ts; for the standard books channel this is "
                "the venue's order-book generation timestamp"
            ),
        },
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--symbols",
        type=_parse_symbols,
        default=_parse_symbols("BTC-USDT-SWAP,ETH-USDT-SWAP,SOL-USDT-SWAP"),
        help="Comma-separated OKX USDT perpetual instrument IDs",
    )
    parser.add_argument("--duration-seconds", type=float, default=60.0)
    parser.add_argument("--book-depth", type=int, choices=(1, 50, 400), default=50)
    parser.add_argument("--rest-interval-seconds", type=float, default=5.0)
    ...
    parser.add_argument("--output-root", type=Path, default=Path("data/live/okx"))
...
def main() -> None:
    ...
    if args.rest_interval_seconds <= 0:
        raise SystemExit("--rest-interval-seconds must be positive")
